...
1. **Single-file сборка** ([`specs/single-file-build/`](specs/single-file-build/spec.md)) — один self-contained
   ...
   **8.3.27+** «ПолеHTMLДокумента пытается открыть локальный файл» (issue #18). Влит в upstream,
   вошёл в релиз 0.5.0.
2. **Апгрейд Monaco 0.20 → 0.55** ([`specs/monaco-0.55/`](specs/monaco-0.55/spec.md)) — подъём редактора
   ...
   BSD-3). Влит в upstream, вошёл в релиз 0.5.0.
3. **Потребление памяти** ([`specs/done/1/`](specs/done/1/result.md), внутренний issue
   [#1](https://github.com/vandalsvq/bsl_console/issues/1)) — разбор расхода памяти в поле 1С.
   Найдена и исправлена утечка: `calculateDiff()` создавал копию модели Monaco на каждое изменение
   текста (4 МБ на правку). Полевая раскладка экземпляра: **61 % — загрузка бандла Monaco ещё до
   создания редактора**, 22 % — создание редактора и наша обвязка, 14 % — словари языка. Три
   направления оптимизации проверены и закрыты измерениями (снос содержимого поля перед закрытием,
   обрезка контрибов Monaco, ленивые словари); эффект остался только на стороне интеграции —
   автозагрузка справки и число экземпляров.
...
| Ветка | Роль |
| --- | --- |
| `local/webpack` | **Рабочая.** Наша линия поверх `webpack`: доработки, спеки, инструменты. Отсюда ведётся разработка и собираются рабочие epf. |
| `webpack` | **Зеркало upstream.** Держится строго равной `upstream/webpack` — своих коммитов не несёт. webpack 5 + **Monaco 0.55.1**. Требует платформу **8.3.24+**. |
| `local/webpack_0_20` | **Рабочая для 0.20.** Наша линия поверх `webpack_0_20` (сейчас — портированный фикс утечки diff-моделей). Отсюда собирается рабочая epf 0.20. |
...
Работа по треку ведётся на своей ветке `feature/<id>-<slug>` от `local/webpack` и вливается в неё
по готовности — в основной линии не работаем напрямую. Ветку для PR автору отпочковывать **от `webpack`** (то есть от чистого upstream) и переносить туда
только относящиеся к задаче коммиты — иначе в PR утащится вся наша линия. Диф в PR держать
минимальным: узкие ревьюибельные PR предпочтительны.
...
**Разные `node_modules` у линий 0.55 и 0.20:** пачки зависимостей не совпадают, после переключения
между `local/webpack` и `local/webpack_0_20` нужен `npm install`, иначе webpack падает на
отсутствующем плагине. `tools/make_epf.ps1` есть только на линии 0.55 — для 0.20 брать оттуда
(`git show local/webpack:tools/make_epf.ps1`).
